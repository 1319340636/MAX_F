...
class DecisionFusionEngine:
    ...
    def __init__(self, agent_ids: List[str]):
        ...
        self.decision_threshold = 1.2  # 默认决策阈值
    ...
    def get_weighted_decision(self, actions: List[Any]) -> Dict:
        ...
        # 阈值判断：多空力量需有明显差距
        if votes["LONG"] > votes["SHORT"] * self.decision_threshold and votes["LONG"] > 0.5:
            final_dir = "LONG"
            # 信心归一化
            final_stake = min(votes["LONG"] / (total_power + 0.1), 1.0)
            
        elif votes["SHORT"] > votes["LONG"] * self.decision_threshold and votes["SHORT"] > 0.5:
            final_dir = "SHORT"
